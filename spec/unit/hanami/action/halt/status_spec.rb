...
RSpec.describe Hanami2::Action::Halt do
  ...
  it "raises error with an unknown integer status code" do
    expect { described_class.(999) }.to raise_exception(Hanami2::Action::UnknownHttpStatusError, /unknown HTTP status: `999'/)
  end

  it "raises error with an unknown symbol status code" do
    expect { described_class.(:foo) }.to raise_exception(Hanami2::Action::UnknownHttpStatusError, /unknown HTTP status: `:foo'/)
  end
end
